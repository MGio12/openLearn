...
Le produit ne doit pas etre filme comme une bibliotheque de fiches. La video doit montrer une boucle courte : diagnostic, mission, preuve, focus, avance.
...
```bash
node scripts/capture-aha-moments.mjs 03-focus-session-progression
```
...
| 3 | Focus -> session terminee -> progression | `focus.html` puis `progression.html` | Timer, bouton terminer, pop-up, puis avance visible. | "Quand tu termines une mission, ton avance prend racine." |
| 4 | Ton avance / plante | `progression.html` | La mission terminee devient une trace visible. | "25 minutes de travail ne disparaissent pas." |
| 5 | Details par matiere | `progression.html` | Le tiroir s'ouvre depuis Ton avance. | "Les stats existent, mais elles restent au bon endroit." |
...
- Montrer les clics importants : diagnostic, pourquoi, terminer la session, details, correction.
