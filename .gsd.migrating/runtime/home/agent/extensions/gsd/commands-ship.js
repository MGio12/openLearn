 ...
 */
export async function checkSliceEvalReview(basePath, milestoneId, sliceId) {
...
export async function handleShip(args, ctx, _pi) {
    ...
    for (const sliceId of listSliceIds(basePath, milestoneId)) {
        ...
        if (result.kind === "absent") {
            ctx.ui.notify(`Slice ${sliceId} has no EVAL-REVIEW.md — consider /gsd eval-review ${sliceId} (non-blocking).`, "warning");
        }
        else if (result.kind === "malformed") {
            ctx.ui.notify(`Slice ${sliceId} EVAL-REVIEW.md frontmatter invalid at ${result.pointer}: ${result.error} (non-blocking).`, "warning");
        }
        else if (result.verdict === "NOT_IMPLEMENTED") {
            ctx.ui.notify(`Slice ${sliceId} eval verdict NOT_IMPLEMENTED (overall ${result.overall_score}/100) — shipping anyway, but the eval gap is unresolved.`, "warning");
        }
    }
