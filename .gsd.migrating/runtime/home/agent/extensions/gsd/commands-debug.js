        ...
        try {
            ...
            if (canDispatch) {
                try {
                    ...
                    pi.sendMessage({ customType: "gsd-debug-start", content: prompt, display: false }, { triggerTurn: true });
                }
                catch (err) {
                    const msg = err instanceof Error ? err.message : String(err);
                    ctx.ui.notify(`Debug dispatch failed: ${msg}\nSession '${s.slug}' is persisted; retry with /gsd debug continue ${s.slug}`, "warning");
                }
            }
        }
        catch (error) {
