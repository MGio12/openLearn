...
function inferFailureKind(message) {
    ...
    if (/stale worker|stale lock|worker.*stale/i.test(message))
        return "stale-worker";
    if (/worktree|\.git|unit root|git metadata/i.test(message))
        return "worktree-invalid";
    if (/verification drift|assessment drift|state drift/i.test(message))
        return "verification-drift";
...
function unitSuffix(input) {
    if (!input.unitType && !input.unitId)
        return "";
