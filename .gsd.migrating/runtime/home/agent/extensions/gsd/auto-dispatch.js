...
export function setResearchProjectPromptBuilderForTest(builder) {
 ...
 */
export function getDeepStageGate(prefs, basePath) {
    return resolveDeepProjectSetupState(prefs, basePath);
}
export function hasPendingDeepStage(prefs, basePath) {
...
export const DISPATCH_RULES = [
    ...
    },
    {
        ...
        match: async ({ state, basePath, prefs }) => {
            ...
            if (isWorkflowPrefsCaptured(basePath))
                return null; // already captured — fall through
    ...
    },
    {
        ...
        match: async ({ state, basePath, prefs, structuredQuestionsAvailable }) => {
            ...
            if (existsSync(projectPath) && validateArtifact(projectPath, "project").ok)
                return null; // PROJECT.md valid — fall through
            return {
                action: "dispatch",
    ...
    },
    {
        ...
        match: async ({ state, basePath, prefs, structuredQuestionsAvailable }) => {
            ...
            const projectPath = join(gsdRoot(basePath), "PROJECT.md");
            if (!existsSync(projectPath) || !validateArtifact(projectPath, "project").ok)
                return null; // PROJECT.md missing/invalid — earlier rule handles
            const requirementsPath = join(gsdRoot(basePath), "REQUIREMENTS.md");
            if (existsSync(requirementsPath) && validateArtifact(requirementsPath, "requirements").ok)
                return null; // REQUIREMENTS.md valid — fall through
            return {
                action: "dispatch",
    ...
    },
    {
        ...
        match: async ({ state, basePath, prefs, structuredQuestionsAvailable }) => {
            ...
            if (gate.status !== "pending" || gate.stage !== "research-decision")
                return null;
            return {
                action: "dispatch",
    ...
    },
    {
        ...
        match: async ({ state, basePath, prefs, structuredQuestionsAvailable }) => {
            ...
            if (gate.status !== "pending" || gate.stage !== "project-research")
                return null;
            ...
            try {
                writeFileSync(inflightMarkerPath, JSON.stringify({ started: new Date().toISOString() }) + "\n", { encoding: "utf-8", flag: "wx" });
            }
            catch (err) {
            ...
            try {
                ...
                try {
                    ...
                    logWarning("dispatch", `failed to remove research-project in-flight marker after prompt assembly error: ${cleanupErr instanceof Error ? cleanupErr.message : String(cleanupErr)}`);
                }
                throw err;
            }
        },
        ...
        match: async ({ state, mid, midTitle, basePath, prefs, sessionContextWindow, modelRegistry, sessionProvider }) => {
            ...
            if (!progressiveOn) {
                ...
                return {
                    ...
                    unitType: "plan-slice",
                    unitId: `${mid}/${sid}`,
                    prompt: await buildPlanSlicePrompt(mid, midTitle, sid, sTitle, basePath, undefined, { ...(softScopeHint ? { softScopeHint } : {}), sessionContextWindow, modelRegistry, sessionProvider }),
                };
            }
            return {
                action: "dispatch",
                ...
                unitId: `${mid}/${sid}`,
                prompt: await buildRefineSlicePrompt(mid, midTitle, sid, sTitle, basePath, undefined, { sessionContextWindow, modelRegistry, sessionProvider }),
            };
        },
    },
    {
        ...
        match: async ({ state, mid, midTitle, basePath, sessionContextWindow, modelRegistry, sessionProvider, session }) => {
            ...
            return {
                ...
                unitType: "plan-slice",
                unitId,
                prompt: await buildPlanSlicePrompt(mid, midTitle, sid, sTitle, basePath, undefined, { sessionContextWindow, modelRegistry, sessionProvider, priorPreExecFailure }),
            };
        },
    },
    {
    ...
    },
    {
        ...
        match: async ({ state, mid, basePath, sessionContextWindow, modelRegistry, sessionProvider }) => {
            ...
            return {
                ...
                unitId: `${mid}/${sid}/${tid}`,
                prompt: await buildExecuteTaskPrompt(mid, sid, sTitle, tid, tTitle, basePath, { sessionContextWindow, modelRegistry, sessionProvider }),
            };
        },
    },
    {
