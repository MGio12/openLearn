...
function hasNestedFileOrSymlink(dir) {
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
        if (entry.isFile() || entry.isSymbolicLink())
            return true;
        if (entry.isDirectory() && hasNestedFileOrSymlink(join(dir, entry.name)))
            return true;
    }
    return false;
}
...
function _getPendingDeepProjectSetup(basePath) {
    ...
    if (pendingDeepProjectSetupMap.size === 1)
        return pendingDeepProjectSetupMap.values().next().value;
    return null;
}
...
function _getPendingDeepProjectSetupForContext(ctx, basePath) {
    ...
    if (!ctx)
        return _getPendingDeepProjectSetup();
...
export function getPendingDeepProjectSetupUnitForContext(ctx, basePath) {
...
export async function startDeepProjectSetupForeground(ctx, pi, basePath, step) {
    const entry = {
        ctx,
        pi,
        basePath,
...
export async function checkDeepProjectSetupAfterTurn(_event, ctx, basePath) {
    const entry = _getPendingDeepProjectSetupForContext(ctx, basePath);
    if (!entry)
        return false;
    if (entry.currentUnitType && entry.currentUnitId) {
...
async function dispatchNextDeepProjectSetupStage(entry) {
    ...
    for (const rule of DISPATCH_RULES) {
        ...
        // complete.
        if (!FOREGROUND_DEEP_SETUP_RULE_NAMES.has(rule.name))
            continue;
