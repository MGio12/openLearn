    ...
    if (s.currentUnit) {
        ...
        if (!opts?.skipWorktreeSync && s.originalBasePath && !isSamePathLocal(s.originalBasePath, s.basePath)) {
            await runSafely("postUnit", "worktree-sync", () => {
                ...
                if (scope)
                    _worktreeProjection.projectWorktreeToRoot(scope);
            });
        }
        ...
        if (s.currentUnit.type === "complete-slice") {
            ...
            await runSafely("postUnit", "slice-cadence-merge", async () => {
                ...
                if (getCollapseCadence(prefs) !== "slice")
                    return;
                if (getIsolationMode(s.originalBasePath || s.basePath) !== "worktree")
                    return;
        ...
        if (!s.currentUnit.type.startsWith("hook/")) {
            ...
            if (!triggerArtifactVerified && s.currentUnit.type === "research-project") {
                ...
                if (triggerArtifactVerified) {
                    invalidateAllCaches();
                    ctx.ui.notify(outcome.kind === "partial-blockers"
                        ? "Project research finished partially; wrote blockers for missing dimensions and advancing without rerunning all scouts."
                        : "Project research artifacts are now terminal.", "warning");
                }
                else {
                    ctx.ui.notify("Project research produced no usable research files; wrote PROJECT-RESEARCH-BLOCKER.md and continuing fail-closed.", "error");
                    return "continue";
                }
            }
            ...
            if (!triggerArtifactVerified && USER_DRIVEN_DEEP_UNITS.has(s.currentUnit.type) && isAwaitingUserInput(opts?.agentEndMessages)) {
                ...
                if (hasExpectedArtifact) {
                    ...
                    ctx.ui.notify(`${failureDetails} Retrying (attempt ${attempt}/${MAX_ARTIFACT_VERIFICATION_RETRIES}).`, "warning");
                    return "retry";
                }
            }
