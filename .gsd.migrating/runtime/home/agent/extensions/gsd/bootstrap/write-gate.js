...
export function shouldBlockPendingGateInSnapshot(snapshot, toolName, _milestoneId, _queuePhaseActive) {
    if (!snapshot.pendingGateId)
        return { block: false };
    if (GATE_SAFE_TOOLS.has(canonicalToolName(toolName)))
        return { block: false };
    return {
        block: true,
        reason: [
 ...
 */
export function shouldBlockRootArtifactSaveInSnapshot(snapshot, artifactType, opts = {}) {
    if (!FINAL_ROOT_ARTIFACTS.has(artifactType))
        return { block: false };
    if (snapshot.pendingGateId) {
        return {
            block: true,
