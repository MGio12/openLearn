...
import { nativeBranchForceReset, nativeBranchExists, nativeCheckoutBranch, nativeCommit, nativeCommitCountBetween, nativeConflictFiles, nativeDetectMainBranch, nativeMergeSquash, } from "./native-git-bridge.js";
...
function resolveIntegrationBranch(projectRoot, milestoneId) {
    ...
    if (recorded && nativeBranchExists(projectRoot, recorded))
        return recorded;
    const prefs = loadEffectiveGSDPreferences(projectRoot)?.preferences?.git ?? {};
    const configured = prefs.main_branch;
    if (configured && nativeBranchExists(projectRoot, configured))
        return configured;
...
function advanceMilestoneBranch(projectRoot, worktreeCwd, milestoneBranch, mainBranch) {
    ...
    if (worktreeCwd !== projectRoot && worktreeBranch === milestoneBranch) {
        ...
        if (status) {
            throw new GSDError(GSD_GIT_ERROR, `slice-cadence cannot advance ${milestoneBranch}: worktree has uncommitted changes. Status:\n${status}`);
        }
...
export function mergeSliceToMain(projectRoot, milestoneId, sliceId) {
    ...
    if (mainBranch === milestoneBranch) {
        throw new GSDError(GSD_GIT_ERROR, `slice-cadence resolved integration branch "${mainBranch}" to the milestone branch; refusing to self-merge.`);
    }
