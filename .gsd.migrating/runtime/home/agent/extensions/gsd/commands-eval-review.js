...
export async function handleEvalReview(args, ctx, pi) {
    ...
    if (!state.activeMilestone) {
        ctx.ui.notify("No active milestone — start or resume one before running /gsd eval-review.", "warning");
        return;
    }
    ...
    if (action.kind === "show") {
        if (!action.path) {
            ctx.ui.notify(`No EVAL-REVIEW.md present for ${parsed.sliceId}. Run /gsd eval-review ${parsed.sliceId} to generate one.`, "warning");
            return;
        }
        try {
    ...
    if (action.kind === "no-summary") {
        ctx.ui.notify(`Slice ${parsed.sliceId} exists but has no SUMMARY.md — run /gsd execute-phase first to generate one.`, "warning");
        return;
    }
    ...
    pi.sendMessage({ customType: "gsd-eval-review", content: prompt, display: false }, { triggerTurn: true });
}
