...
export function _enterMilestoneCore(s, deps, milestoneId, ctx) {
    ...
    if (s.workerId) {
        if (s.currentMilestoneId === milestoneId &&
            s.milestoneLeaseToken !== null) {
            const refreshed = refreshMilestoneLease(s.workerId, milestoneId, s.milestoneLeaseToken);
            if (refreshed) {
                debugLog("WorktreeLifecycle", {
                    action: "enterMilestone",
                    milestoneId,
...
export class WorktreeLifecycle {
    ...
    mergeAndEnterNext(currentMilestoneId, nextMilestoneId, ctx) {
        ...
        try {
            ...
            if (this.s.basePath !== projectRoot)
                throw err;
        ...
        if (!merged && !mergeThrew && !this.s.isolationDegraded) {
            ...
            throw new Error(`Cannot enter milestone ${nextMilestoneId} because ${currentMilestoneId} was not merged`);
        }
