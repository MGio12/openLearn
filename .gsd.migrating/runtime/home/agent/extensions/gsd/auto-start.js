 ...
 */
export function _finalizeSurvivorBranch(lifecycle, milestoneId, ui) {
    ui.notify(`Milestone ${milestoneId} is complete but branch/worktree was not finalized. Running merge now.`, "info");
    const result = lifecycle.exitMilestone(milestoneId, { merge: true }, { notify: ui.notify.bind(ui) });
    if (result.ok)
        return { merged: true };
 ...
 */
export function _mergeOrphanCompletedMilestone(lifecycle, orphanId, ui) {
    ui.notify(`Detected unmerged completed milestone ${orphanId}. Merging now.`, "info");
    const result = lifecycle.exitMilestone(orphanId, { merge: true }, { notify: ui.notify.bind(ui) });
    if (result.ok)
        return { merged: true };
    const err = result.cause instanceof Error ? result.cause : new Error(String(result.cause));
    const msg = err.message;
...
export async function bootstrapAutoSession(s, ctx, pi, base, verboseMode, requestedStepMode, deps, interrupted) {
    ...
    try {
        ...
        if (existsSync(gitLockFile)) {
            ctx.ui.notify("Git index lock is present at .git/index.lock. Another git process may be running; resolve the lock before starting GSD.", "error");
        ...
        if (s.currentMilestoneId &&
            ...
            if (!enterResult.ok) {
                ...
                if (enterResult.reason === "lease-conflict") {
                    ...
                    ctx.ui.notify(`Auto-mode bootstrap stopped: failed to enter milestone ${s.currentMilestoneId} (${enterResult.reason}).`, "error");
                }
                return releaseLockAndReturn();
            }
            if (s.basePath !== base) {
        ...
        const scopeMsg = deepProjectStagePending
        ...
        if (providerReportedWindow > 500_000 && contextOverride === undefined) {
            ctx.ui.notify(`Model reports a ${Math.round(providerReportedWindow / 1000)}K context window. If the provider's real API limit is lower, set context_window_override in .gsd/PREFERENCES.md so wrap-up signals fire before context overflow.`, "warning");
        }
