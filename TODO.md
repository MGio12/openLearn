...
### 3bis. Porte d'entree compte via onboarding

- [ ] Faire de `onboarding.html` la porte d'entree unique pour creer un compte local.

Objectif : clarifier le parcours compte avant l'IA. Sans compte local, l'utilisateur passe par `onboarding.html` et le dossier actif `onboarding/`. Avec un compte local existant, `onboarding.html` renvoie directement vers `index.html`.

Actions :

- Confirmer que le compte local est detecte via `window.OutilPrepa.hasLocalAccount()` et `profile.localAccountId`.
- Faire de `onboarding.html` le seul point de creation de compte local.
- Garder `parametres.html` pour modifier ou reinitialiser un profil deja cree, pas comme entree de creation par defaut.
- Travailler sur l'onboarding actif : `onboarding/*.jsx` et le bundle genere `onboarding/onboarding.bundle.js`.
- Ne pas modifier l'ancien `assets/js/pages/onboarding.js` comme source d'onboarding actif.
- Remplacer les fallbacks sans compte des pages principales pour pointer vers `onboarding.html` uniquement.
- Ajouter dans `onboarding.html` une redirection vers `index.html` quand `window.OutilPrepa.hasLocalAccount()` vaut deja vrai.
- Garder `index.html` comme destination unique apres detection d'un compte local existant.
...
- Sans compte local, les pages principales menent toutes vers `onboarding.html`.
- Avec un compte local, ouvrir `onboarding.html` renvoie vers `index.html`.
- `parametres.html` reste utile apres creation du compte pour modifier ou reinitialiser le profil.
- Le bundle `onboarding/onboarding.bundle.js` est regenere si un JSX est modifie.
- Un commit est cree apres les verifications et le garde-fou doc-impact-review ; aucun commit n'est cree si une verification liee a l'item echoue.

### 5. Composant Feynman Methode

- [ ] Construire un composant reutilisable "Feynman Methode" a la fin d'un cours.

Objectif : l'eleve ecrit ce qu'il pense avoir compris, puis recoit une correction claire et utile.

Actions :

- Concevoir une zone blanche de redaction a la fin du cours :
  - textarea large et lisible ;
  - bouton d'envoi ;
  - etat chargement ;
  - resultat IA ;
  - bouton recommencer ;
  - sauvegarde locale de la derniere tentative par cours.
- Le bouton visible dans le cours doit etre clair : "Methode Feynman" ou "Expliquer le cours avec mes mots".
- Ne pas remplacer la correction du cours : l'IA aide a diagnostiquer et approfondir.
- Prevoir un fallback sans IA : sauvegarder le texte et afficher une consigne de relecture.

Tester :

- Test manuel sur un cours :
  - texte vide ;
  - texte court ;
  - texte avec erreur ;
  - texte long mais accepte ;
  - reponse mock ;
  - reponse API si cle configuree.
...
- L'eleve peut ecrire, envoyer, lire un retour, corriger sa reponse et recommencer.
- L'interface ne cree pas d'overflow mobile.
- Les cours restent utilisables sans cle API.

### 6. Prompt Feynman clair et pedagogique

- [ ] Definir le prompt systeme et le format de reponse Feynman.

Objectif : obtenir des retours IA stables, clairs, exigeants, et utiles a un lyceen.

Actions :

- Definir un format de sortie stable :
  - ce que l'eleve a compris ;
  - erreurs ou confusions ;
  - notions manquees ;
  - explication claire ;
  - question de verification ;
  - prochaine micro-action.
- Le prompt doit interdire :
  - inventer du contenu absent du cours sans le signaler ;
  - donner une note humiliante ;
  - promettre un resultat scolaire garanti ;
  - ignorer les erreurs mathematiques.
- Pour les maths, exiger des notations stables, des hypotheses explicites, et des corrections structurees.
- Prevoir un mode "simple", "normal", "exigeant", et "coach calme" sans multiplier les agents techniques au debut.

Tester :

- Tests de prompt avec exemples fixes :
  - reponse eleve presque vide ;
  - reponse eleve partiellement juste ;
  - reponse eleve fausse ;
  - reponse eleve correcte mais incomplete ;
  - demande de ton different.
- Verifier que le retour reste actionnable et non generique.
- `git diff --check`

Criteres de fini :

- Le retour IA suit toujours la structure attendue.
- Le ton varie sans changer l'exigence mathematique.
- L'eleve sait quoi refaire apres lecture.
