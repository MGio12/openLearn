...
function initReadingProgress() {
  ...
  const updateScrollProgress = () => {
    ...
    const heroBottom = document.querySelector('.course-hero')?.getBoundingClientRect().bottom ?? 0;
...
function initLazyCourseGraphs() {
  const boards = Array.from(document.querySelectorAll([
    '[data-chapter-curve]',
    '[data-course-curve]',
    '[data-derivative-board]',
    '[data-rate-board]',
    '[data-sign-board]',
  ].join(',')));
...
function runGraphInitializer(element) {
  if (element.hasAttribute('data-chapter-curve')) {
    const initializers = window.__courseGraphInitializers || [];
    for (const initializer of initializers) {
      initializer(element);
      if (element.dataset.courseGraphReady === 'true') return;
    }
    setGraphFallback(element);
    return;
  }

  if (element.hasAttribute('data-derivative-board')) {
    initDerivativeBoard(element);
    return;
  }

  if (element.hasAttribute('data-rate-board')) {
    initRateBoard(element);
    return;
  }

  if (element.hasAttribute('data-sign-board')) {
    initSignBoard(element);
    return;
  }

  if (element.hasAttribute('data-course-curve')) {
